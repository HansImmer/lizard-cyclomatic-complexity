...
export default {
  ...
  provideLinter () {
    return {
      ...
      scope: 'file',
      lintsOnChange: false,
      grammarScopes: ['source.python', 'source.python.django', 'source.c', 'source.cpp', 'source.objc', 'source.objcpp', 'text.html.php', 'source.php'],
      lint: async (editor) => {
        ...
        for (var line of analyzedfunctionsString) {
          ...
          const functionLength = parseInt(m[5]);
          const functionName = m[6];
          const lineNumberInEditor = parseInt(m[7]);
          ...
          if ((cyclomaticComplexity > this.thresholdCyclomaticComplexity) ||
            ...
            (functionLength > this.thresholdlinesOfCodeWithoutComments) ||
            (functionLength > this.thresholdlinesOfCodeWithoutComments) ||
            (functionLength > this.thresholdNumberOfTokens)) {
            position = generateRange(editor, lineNumberInEditor);
